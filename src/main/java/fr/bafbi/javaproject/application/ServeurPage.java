...
//            int nbRecettes = restaurant.getRecettes().size();
        ...
        //route permettant de terminer une transaction (payement de la note) (possiblement en plusieurs fois)
        app.get("/serveur/{serveurId}/{transactionId}/terminer", ctx -> {
            var serveurId = Integer.parseInt(ctx.pathParam("serveurId"));
            var transactionId = Integer.parseInt(ctx.pathParam("transactionId"));
            var transaction = restaurant.getTransactionManager().getTransaction(transactionId);
            ...
            var content = html(
                    ...
                    body(
                            Application.HeaderElement(),
                            h1("Payement de la transaction " + transactionId),
                            // show the command, with the price
                            ul(
                                    each(transaction.getCommand().getRecettesCommandes().keySet(), recette -> li(recette.getName() + " x" + transaction.getCommand().getRecettesCommandes().get(recette) + " - " + recette.getPrice() * transaction.getCommand().getRecettesCommandes().get(recette) + "€")),
                                    each(transaction.getCommand().getBoissonsCommandes().keySet(), boisson -> li(boisson.name() + " x" + transaction.getCommand().getBoissonsCommandes().get(boisson) + " - " + boisson.getPrice() * transaction.getCommand().getBoissonsCommandes().get(boisson) + "€"))
                            ),
                            h2("Total : " + transaction.getCommand().getTotalPrice() + "€"),
                            // form to pay
                            div(
                                    h3(attrs("#reste"), "Reste à payer : " + transaction.getMontantRestant() + "€"),
                                    form(
                                            div(
                                                    label("Montant : "),
                                                    input().withName("montant").withPlaceholder("Montant").isRequired()
                                            ),
                                            button("Payer").withType("submit")
                                    )
                                            .attr("hx-post", "/serveur/" + serveurId + "/" + transactionId + "/terminer")
                                            .attr("hx-target", "#reste")
                                            .attr("hx-swap", "outerHTML"),
                                    // pay le reste
                                    button("Payer le reste").withType("submit")
                                                            .attr("hx-post", "/serveur/" + serveurId + "/" + transactionId + "/terminer")
                                                            .attr("hx-target", "#reste")
                                                            .attr("hx-swap", "outerHTML")
                            )
        ...
        // handle payement
        app.post("/serveur/{serveurId}/{transactionId}/terminer", ctx -> {
            var serveurId = Integer.parseInt(ctx.pathParam("serveurId"));
            var transactionId = Integer.parseInt(ctx.pathParam("transactionId"));
            var transaction = restaurant.getTransactionManager().getTransaction(transactionId);


            var montant = Double.parseDouble(Optional.ofNullable(ctx.formParam("montant")).orElse(Double.toString(transaction.getMontantRestant())));

            if (montant <= transaction.getMontantRestant()) {
                transaction.payer(montant);
            }
            // save the transaction if it's finished
            if (transaction.getMontantRestant() == 0) {
                restaurant.createFacture(transaction);
                restaurant.getTransactionManager().removeTransaction(transactionId);
                ctx.res().addHeader("HX-Redirect", "/serveur/" + serveurId);
            }
            ctx.html(h3(attrs("#reste"), "Reste à payer : " + transaction.getMontantRestant() + "€").render());
        });
    ...
    private static DivTag transactionElement(Transaction transaction, int serveurId) {
        ...
        return div(attrs(".transaction intradiv2"),
                ...
                div(attrs(".grid grid-cols-2 gap-2"),
                        ...
                        a("Terminer").withHref("/serveur/" + serveurId + "/" + transaction.getId() + "/terminer"))
        );
    }
